// api/index.js - BRONX OSINT V9.0 - MULTI-KEY + DAILY LIMIT + PROTECTION
...
let cooldownTimers = {};
let ipMinuteCount = {};
let protectedData = {}; // 🔥 Protected numbers/usernames
let dailyLimits = {};   // 🔥 Daily usage tracking
...
function checkDailyLimit(k) {
    ...
    if (dailyLimits[dk] >= kd.dailyLimit) return { allowed: false, remaining: 0, message: `🔴 Daily Limit ${kd.dailyLimit}/${kd.dailyLimit} Reached! Resets at 12 AM.` };
...
app.get('/test',(req,res)=>{res.json({status:'✅ BRONX V9',storage:'RENDER DISK',endpoints:Object.keys(endpoints).length,total_keys:Object.keys(keyStorage).length})});

// 🔥 PROTECTION CHECK IN API
app.get('/api/leakinfo',async(req,res)=>{try{const t=req.query.term||req.query.info;if(!t)return res.json({error:'Missing term'});const prot=isProtected(t);if(prot)return res.json({error:'🔒 This data is PROTECTED! No information available.',protected:true});const r=await axios.get(`${REAL_API_BASE}/leakinfo?key=${getNextKey()}&info=${encodeURIComponent(t)}`,{timeout:30000});res.json({...sanitizeResponse(r.data),api_info:{endpoint:'leakinfo'}})}catch(e){res.json({error:'API error'})}});

app.get('/api/custom/:ep',async(req,res)=>{try{const api=customAPIs.find(a=>a.endpoint===req.params.ep&&a.visible);if(!api)return res.json({error:'Not found'});const key=req.query.key;if(!key)return res.json({error:'Key required'});const kc=checkKeyValid(key);if(!kc.valid)return res.json({error:kc.error});const sc=checkKeyScope(kc.keyData,'custom');const sc2=checkKeyScope(kc.keyData,req.params.ep);if(!sc.valid&&!sc2.valid)return res.json({error:'Scope denied. Add custom scope to key.'});const pv=req.query[api.param]||req.query.number;if(!pv)return res.json({error:'Missing param'});const prot=isProtected(pv);if(prot)return res.json({error:'🔒 This data is PROTECTED!',protected:true});let url=api.realAPI.replace(/\{param\}/gi,encodeURIComponent(pv));const r=await axios.get(url,{timeout:30000});incrementKeyUsage(key);logReq(key,'c/'+req.params.ep,pv,'success',req.clientIP);res.json({...sanitizeResponse(r.data),api_info:{key_owner:kc.keyData?.name,remaining:kc.keyData?.unlimited?'∞':Math.max(0,(kc.keyData?.limit||0)-(kc.keyData?.used||0)),dailyRemaining:kc.keyData?.dailyLimit?Math.max(0,kc.keyData.dailyLimit-(dailyLimits[key+'_'+getIndiaDate()]||0)):'∞',limit:kc.keyData?.unlimited?'∞':kc.keyData?.limit,used:kc.keyData?.used||0,created:kc.keyData?.created,expiry:kc.keyData?.expiryStr||'LIFETIME'}})}catch(e){res.json({error:'API error'})}});

app.get('/api/key-bronx/:ep',async(req,res)=>{try{const ep=req.params.ep;if(!endpoints[ep])return res.json({error:'Not found'});const key=req.query.key;if(!key)return res.json({error:'Key required'});const kc=checkKeyValid(key);if(!kc.valid)return res.json({error:kc.error});const sc=checkKeyScope(kc.keyData,ep);if(!sc.valid)return res.json({error:sc.error});const pv=req.query[endpoints[ep].p];if(!pv)return res.json({error:'Missing '+endpoints[ep].p});const prot=isProtected(pv);if(prot)return res.json({error:'🔒 This '+ep+' data is PROTECTED!',protected:true});const url=`${REAL_API_BASE}/${ep}?key=${getNextKey()}&${endpoints[ep].p}=${encodeURIComponent(pv)}`;const r=await axios.get(url,{timeout:30000});incrementKeyUsage(key);logReq(key,ep,pv,'success',req.clientIP);res.json({...sanitizeResponse(r.data),api_info:{key_owner:kc.keyData?.name,remaining:keyStorage[key]?.unlimited?'∞':Math.max(0,(keyStorage[key]?.limit||0)-(keyStorage[key]?.used||0)),dailyRemaining:keyStorage[key]?.dailyLimit?Math.max(0,keyStorage[key].dailyLimit-(dailyLimits[key+'_'+getIndiaDate()]||0)):'∞',limit:keyStorage[key]?.unlimited?'∞':keyStorage[key]?.limit,used:keyStorage[key]?.used||0,created:keyStorage[key]?.created,expiry:keyStorage[key]?.expiryStr||'LIFETIME'}})}catch(e){res.json({error:'API error'})}});
...
// 🔥 PROTECTION ROUTES
app.post('/admin/add-protection',async(req,res)=>{if(!isAdminAuth(req.headers['x-admin-token']||req.query.token))return res.json({e:'Unauthorized'});const{value,type}=req.body;if(!value)return res.json({e:'Missing'});protectedData[value]=value;saveToDisk();res.json({success:true,message:'✅ Protected: '+value})});
...
// ========== GOLD THEME LOGIN ==========
function renderLogin(){return `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>BRONX V9 | GOLD</title><link href="https://fonts.googleapis.com/css2?family=Orbitron:wght@400;700;900&family=Rajdhani:wght@300;500;700&display=swap" rel="stylesheet"><style>
...
.gold-bg{position:fixed;inset:0;pointer-events:none;z-index:0}.gold-orb{position:absolute;border-radius:50%;filter:blur(100px);animation:float 8s infinite}.gold-orb:nth-child(1){width:400px;height:400px;background:rgba(255,180,0,.15);top:-10%;left:-5%}.gold-orb:nth-child(2){width:350px;height:350px;background:rgba(255,200,0,.1);bottom:-10%;right:-5%;animation-delay:3s}@keyframes float{0%,100%{transform:translate(0,0)}33%{transform:translate(40px,-40px)}66%{transform:translate(-30px,30px)}}
.card{background:rgba(10,8,0,.9);padding:55px 45px;border-radius:28px;width:450px;position:relative;z-index:1;backdrop-filter:blur(50px);border:1px solid rgba(255,180,0,.15);box-shadow:0 0 120px rgba(255,180,0,.08)}.card::after{content:'';position:absolute;inset:-1px;border-radius:28px;padding:1px;background:linear-gradient(135deg,rgba(255,180,0,.4),rgba(255,200,0,.2),rgba(200,150,0,.3));-webkit-mask:linear-gradient(#fff 0 0) content-box,linear-gradient(#fff 0 0);mask:linear-gradient(#fff 0 0) content-box,linear-gradient(#fff 0 0);-webkit-mask-composite:xor;mask-composite:exclude;pointer-events:none}
...
.card input{width:100%;padding:17px 20px;background:rgba(0,0,0,.6);border:1px solid rgba(255,255,255,.04);border-radius:16px;color:#fff;font-size:15px;outline:none;font-family:'Rajdhani',sans-serif;transition:.4s;margin-bottom:16px}.card input:focus{border-color:#ffb400}.card input::placeholder{color:#2a2a00}
.btn{width:100%;padding:18px;background:linear-gradient(135deg,#ffb400,#ffd700,#ff8c00);color:#000;border:none;border-radius:16px;cursor:pointer;font-size:16px;font-weight:700;letter-spacing:5px;font-family:'Orbitron',sans-serif;animation:goldGlow 2s ease infinite}.btn:hover{transform:translateY(-3px);box-shadow:0 0 80px rgba(255,180,0,.5)}@keyframes goldGlow{0%,100%{background-position:0% 50%}50%{background-position:100% 50%}}
.msg{color:#ff8c00;text-align:center;margin-top:14px;font-size:13px;display:none;font-weight:600}.foot{text-align:center;margin-top:24px;font-size:10px;color:#2a1a00;letter-spacing:4px}
</style></head><body><div class="gold-bg"><div class="gold-orb"></div><div class="gold-orb"></div><div class="gold-orb"></div></div><div class="card"><div class="logo">BRONX OSINT V9.0</div><h2>👑 GOLD ACCESS</h2><p class="sub">Multi-Key · Daily Limit · Protection</p><input type="text" id="u" placeholder="USERNAME"><input type="password" id="p" placeholder="PASSWORD"><button class="btn" onclick="login()">AUTHENTICATE</button><p class="msg" id="msg"></p><p class="foot">BRONX V9 · GOLD EDITION</p></div><script>async function login(){var u=document.getElementById('u').value,p=document.getElementById('p').value,m=document.getElementById('msg');if(!u||!p){m.style.display='block';m.style.color='#ffaa00';m.textContent='⚠ Fill all fields';return}m.style.display='block';m.style.color='#ffb400';m.textContent='👑 Authenticating...';try{var r=await fetch('/admin/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({username:u,password:p})});var d=await r.json();if(d.success){m.style.color='#ffd700';m.textContent='✓ '+d.message;setTimeout(()=>location.href=d.redirect,500)}else{m.style.color='#ff4444';m.textContent='✗ '+d.error}}catch(e){m.textContent='✗ Connection error'}}</script></body></html>`}
...
let keysHTML=allKeys.map(k=>{let s='ACTIVE',sc='#ffd700';if(k.isExpired){s='EXPIRED';sc='#8b7500'}else if(k.left==0){s='LIMIT';sc='#ff8c00'}const sd=k.scopes.includes('*')?'ALL':k.scopes.slice(0,2).join(',')+(k.scopes.length>2?'..':'');return`<tr><td><code>${esc(k.key.substring(0,12))}${k.key.length>12?'..':''}</code></td><td>${esc(k.name)}</td><td>${k.limit}</td><td>${k.used}</td><td>${k.dailyLimit||'∞'}</td><td style="color:${k.left==0?'#ff0000':'#ffd700'}">${k.left}</td><td>${esc(k.expiry)}</td><td style="color:#ffb400">${sd}</td><td style="color:${sc}">${s}</td><td>${esc(k.created||'')}</td><td style="text-align:center"><button class="ab a-g" onclick="resetKey('${esc(k.key)}')">↺</button><button class="ab a-y" onclick="pushKey('${esc(k.key)}')">⬆</button><button class="ab a-r" onclick="stopKey('${esc(k.key)}')">⛔</button><button class="ab a-r" onclick="deleteKey('${esc(k.key)}')">✕</button></td></tr>`}).join('');
...
const protHTML=Object.keys(protectedData).map(v=>`<tr><td><code>${esc(v)}</code></td><td>🔒 Protected</td><td><button class="ab a-r" onclick="removeProt('${esc(v)}')">✕</button></td></tr>`).join('')||'<tr><td colspan="3" style="color:#444;text-align:center">No protected data</td></tr>';

return `<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>BRONX V9 | GOLD PANEL</title><link href="https://fonts.googleapis.com/css2?family=Orbitron:wght@400;700;900&family=Rajdhani:wght@300;400;500;600;700&display=swap" rel="stylesheet"><style>
:root{--bg:#0a0a00;--sur:rgba(15,12,0,.8);--brd:rgba(255,180,0,.08);--txt:#e0d8c0;--acc:#ffb400;--acc2:#ffd700;--green:#ffd700;--red:#8b7500;--yellow:#ff8c00}
...
body::before{content:'';position:fixed;inset:0;background:radial-gradient(ellipse at 50% -10%,rgba(255,180,0,.1),transparent 50%);pointer-events:none;z-index:0}
::-webkit-scrollbar{width:3px}::-webkit-scrollbar-track{background:#0a0a00}::-webkit-scrollbar-thumb{background:var(--acc)}
...
.scope-box{display:flex;flex-wrap:wrap;gap:6px;padding:10px;background:rgba(0,0,0,.3);border:1px solid var(--brd);border-radius:10px;max-height:120px;overflow:auto}.scope-box label{cursor:pointer;font-size:10px;color:#887700}
@media(max-width:768px){.stats-grid{grid-template-columns:repeat(2,1fr)}.fgrid{grid-template-columns:1fr}}
</style></head><body>
<div class="top"><h1>👑 BRONX GOLD V9</h1><div class="tb"><span style="color:#2a1a00;font-size:9px;font-family:monospace">${getIndiaDateTime()}</span><a href="/">🏠 HOME</a><a href="/docs">📚 DOCS</a><a href="/admin">🚪 LOGOUT</a></div></div>
<div class="container">
...
<div class="stat-card"><div class="val">${Object.keys(protectedData).length}</div><div class="lbl">Protected</div></div>
</div>
<div class="tabs">
...
<div class="tab" onclick="st('protect')">🔒 PROTECT</div>
<div class="tab" onclick="st('apis')">🔧 APIs</div>
<div class="tab" onclick="st('addapi')">➕ ADD</div>
<div class="tab" onclick="st('settings')">⚙ SET</div>
</div>

<div class="panel on" id="panel-gen"><div class="section"><h3>⚡ GENERATE KEY</h3><div class="fgrid">
<div><label>Key ID</label><input id="gk" placeholder="MY_KEY"></div>
<div><label>Owner</label><input id="go" placeholder="Name"></div>
<div><label>Total Limit</label><input id="gl" value="100" type="number"></div>
<div><label>Daily Limit (0=∞)</label><input id="gdl" value="0" type="number"></div>
...
<div class="panel" id="panel-protect"><div class="section"><h3>🔒 PROTECTION (Hide Data)</h3><div class="fgrid"><div><label>Value (Number/Username)</label><input id="protVal" placeholder="9876543210"></div><div style="grid-column:1/-1"><button class="btn-gold" onclick="addProt()" style="width:100%">🔒 ADD PROTECTION</button></div></div><br><div style="max-height:300px;overflow:auto"><table><tr><th>VALUE</th><th>STATUS</th><th>ACT</th></tr>${protHTML}</table></div></div></div>
...
<div class="panel" id="panel-settings"><div class="section"><h3>⚙ SETTINGS</h3><button class="btn-gold" onclick="resetAll()" style="width:100%;margin-bottom:10px">🔄 RESET ALL USAGE</button><button class="ab a-r" onclick="if(confirm('Delete ALL?'))deleteAllKeys()" style="width:100%;padding:12px">🗑 DELETE ALL KEYS</button></div></div>
...
async function gk(){var n=document.getElementById('gk').value.trim(),o=document.getElementById('go').value.trim();if(!n||!o){alert('Fill fields');return}var scopes=[];if(document.getElementById('scope-all').checked)scopes=['*'];else document.querySelectorAll('.scope-cb:checked').forEach(c=>scopes.push(c.value));var r=await ac('/admin/generate-key',{keyName:n,keyOwner:o,scopes:scopes,limit:document.getElementById('gl').value,dailyLimit:parseInt(document.getElementById('gdl').value)||0,days:parseInt(document.getElementById('gd').value)||30,cooldown:parseInt(document.getElementById('gc').value)||0});r.success?(alert('✅ Key: '+n+' | Daily: '+(document.getElementById('gdl').value||'∞')),location.reload()):alert('❌ '+(r.e||'Error'))}
...
async function removeProt(v){if(!confirm('Remove protection?'))return;await ac('/admin/remove-protection',{value:v});location.reload()}
...
function renderDocs(){return `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>V9 DOCS</title><link href="https://fonts.googleapis.com/css2?family=Orbitron:wght@400;700;900&family=Rajdhani:wght@400;500;600&display=swap" rel="stylesheet"><style>
:root{--bg:#0a0a00;--sur:rgba(15,12,0,.7);--brd:rgba(255,180,0,.08);--txt:#e0d8c0;--acc:#ffb400}
...
.card{background:var(--sur);border:1px solid var(--brd);border-radius:14px;padding:14px}.method{background:rgba(255,215,0,.1);color:#ffd700;padding:2px 8px;border-radius:4px;font-size:10px}.card b{color:#fff;font-size:14px;margin-left:6px}.card p{color:#887700;font-size:10px;margin:4px 0}
code{display:block;background:rgba(0,0,0,.3);color:#ffb400;padding:6px;border-radius:4px;font-size:9px;margin:6px 0;font-family:monospace}
</style></head><body><div class="top"><a href="/">BRONX V9</a><a href="/">HOME</a><a href="/admin">ADMIN</a></div><div class="ct"><div class="hero"><h1>API DOCS V9 GOLD</h1></div><div class="grid">${Object.entries(endpoints).map(([n,e])=>`<div class="card"><span class="method">GET</span><b>/${n}</b><p>${e.d}</p><code>GET /api/key-bronx/${n}?key=KEY&${e.p}=${e.e}</code></div>`).join('')}</div></div></body></html>`}

function renderHome(){const vapi=customAPIs.filter(a=>a.visible);const tEP=Object.keys(endpoints).length+vapi.length;let cards='';Object.entries(endpoints).forEach(([n,e])=>{cards+=`<div class="ep" onclick="cp('${esc(n)}','${esc(e.p)}','${esc(e.e)}')"><span>${e.i}</span><b>/${esc(n)}</b><small>${e.d}</small><code>${e.p}=${e.e}</code></div>`});vapi.forEach(a=>{cards+=`<div class="ep" style="--ac:#ffb400" onclick="ccp('${esc(a.endpoint)}','${esc(a.param)}','${esc(a.example)}')"><span>🔧</span><b>/${esc(a.endpoint)}</b><small>Custom</small><code>${a.param}=${a.example||'v'}</code></div>`});return `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>BRONX OSINT V9</title><link href="https://fonts.googleapis.com/css2?family=Orbitron:wght@400;700;900&family=Rajdhani:wght@300;400;600;700&display=swap" rel="stylesheet"><style>
:root{--bg:#0a0a00;--sur:rgba(15,12,0,.6);--brd:rgba(255,180,0,.06);--txt:#e0d8c0;--acc:#ffb400}
...
</style></head><body><nav><a href="/" class="logo">BRONX V9</a><div><a href="/docs">DOCS</a> <a href="/admin">ADMIN</a></div></nav><header class="hero"><h1>BRONX OSINT V9.0 👑</h1><p style="color:#887700">Multi-Key · Daily Limit · Protection</p></header><div class="container"><div class="grid">${cards}</div></div><footer><p class="fb">BRONX OSINT V9.0 GOLD</p></footer><script>var eps=${JSON.stringify(endpoints)};function cp(n,p,e){navigator.clipboard.writeText(location.origin+'/api/key-bronx/'+n+'?key=KEY&'+p+'='+e)}function ccp(n,p,e){navigator.clipboard.writeText(location.origin+'/api/custom/'+n+'?key=KEY&'+p+'='+(e||'v'))}</script></body></html>`}

const PORT = process.env.PORT || 3000;
(async function(){initHardcodedKeys();if(!loadFromDisk()){if(customAPIs.length===0)initCustomAPIs()}if(!keyStorage[MASTER_API_KEY])keyStorage[MASTER_API_KEY]=createMasterKey();scheduleSave();app.listen(PORT,()=>{console.log('👑 BRONX OSINT V9.0 GOLD ONLINE!');console.log('🚀 PORT: '+PORT)})})();
module.exports = app;
